...
func (a *API) CheckLogin() bool {
	logging.Log(a.logger, slog.LevelDebug, "checkLogin sending", "api")
	req, err := http.NewRequest("GET", "https://my.115.com/?ct=guide&ac=status", nil)
	if err != nil {
		logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("checkLogin failed to build request err=%v", err), "api")
		return false
	}
	req.Header.Set("Cookie", a.cookies)
	...
	if err != nil {
		logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("checkLogin network error err=%v", err), "api")
		return false
	}
	if resp.StatusCode != http.StatusOK {
		resp.Body.Close()
		logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("checkLogin unexpected status=%d", resp.StatusCode), "api")
		return false
	}
	defer resp.Body.Close()
	var result map[string]any
	if err := json.NewDecoder(resp.Body).Decode(&result); err != nil {
		logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("checkLogin JSON decode error err=%v", err), "api")
		return false
	}
...
func (a *API) RenewCookies(app string) bool {
	if a.cookies == "" {
		logging.Log(a.logger, slog.LevelDebug, "renewCookies no cookies, skipping", "api")
		return false
	}
	if app == "" {
		app = "tv"
	...
	// Step 1: get token.
	logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("renewCookies step1 fetching token app=%s", app), "api")
	resp, err := doGet(QRAPI+"/api/1.0/"+app+"/1.0/token/", nil, "")
	if err != nil {
		logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("renewCookies step1 failed err=%v", err), "api")
		return false
	}
	defer resp.Body.Close()
	var tokenResult map[string]any
	if json.NewDecoder(resp.Body).Decode(&tokenResult) != nil {
		logging.Log(a.logger, slog.LevelDebug, "renewCookies step1 JSON decode failed", "api")
		return false
	}
	tokenData, _ := tokenResult["data"].(map[string]any)
	if tokenData == nil {
		logging.Log(a.logger, slog.LevelDebug, "renewCookies step1 no token data", "api")
		return false
	}
	uid, _ := tokenData["uid"].(string)
	if uid == "" {
		logging.Log(a.logger, slog.LevelDebug, "renewCookies step1 empty uid", "api")
		return false
	}
	logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("renewCookies step1 ok uid=%s", uid), "api")

	// Step 2: prompt.
	logging.Log(a.logger, slog.LevelDebug, "renewCookies step2 prompt", "api")
	resp2, err := doGet(QRAPI+"/api/2.0/prompt.php", url.Values{"uid": {uid}}, a.cookies)
	if err != nil {
		logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("renewCookies step2 failed err=%v", err), "api")
		return false
	}
	resp2.Body.Close()
	logging.Log(a.logger, slog.LevelDebug, "renewCookies step2 ok", "api")

	// Step 3: slogin.
	logging.Log(a.logger, slog.LevelDebug, "renewCookies step3 slogin", "api")
	resp3, err := doGet(QRAPI+"/api/2.0/slogin.php",
		url.Values{"key": {uid}, "uid": {uid}, "client": {"0"}}, a.cookies)
	if err != nil {
		logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("renewCookies step3 failed err=%v", err), "api")
		return false
	}
	resp3.Body.Close()
	logging.Log(a.logger, slog.LevelDebug, "renewCookies step3 ok", "api")

	// Step 4: exchange for new cookies.
	logging.Log(a.logger, slog.LevelDebug, "renewCookies step4 exchange for new cookies", "api")
	formData := url.Values{"account": {uid}, "app": {app}}
	req, err := http.NewRequest("POST", PassportAPI+"/app/1.0/"+app+"/1.0/login/qrcode",
		strings.NewReader(formData.Encode()))
	if err != nil {
		logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("renewCookies step4 build request failed err=%v", err), "api")
		return false
	}
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	resp4, err := client.Do(req)
	if err != nil {
		logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("renewCookies step4 request failed err=%v", err), "api")
		return false
	}
	defer resp4.Body.Close()
	var loginResult map[string]any
	if json.NewDecoder(resp4.Body).Decode(&loginResult) != nil {
		logging.Log(a.logger, slog.LevelDebug, "renewCookies step4 JSON decode failed", "api")
		return false
	}
	data, _ := loginResult["data"].(map[string]any)
	if data == nil {
		logging.Log(a.logger, slog.LevelDebug, "renewCookies step4 no data in response", "api")
		return false
	}
	cookies, _ := data["cookie"].(map[string]any)
	if len(cookies) == 0 {
		logging.Log(a.logger, slog.LevelDebug, "renewCookies step4 empty cookies", "api")
		return false
	}
	var parts []string
	...
	if len(cookieStr) < 10 {
		logging.Log(a.logger, slog.LevelDebug, "renewCookies step4 cookie string too short", "api")
		return false
	}
	a.cookies = cookieStr
	...
	logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("renewCookies success cookieLen=%d", len(cookieStr)), "api")
	return true
}
...
func (a *API) QRLogin(app string) error {
	...
	logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("qrLogin fetching token app=%s", app), "api")
	client := &http.Client{Timeout: 35 * time.Second}

	resp, err := client.Get(QRAPI + "/api/1.0/" + app + "/1.0/token/")
	if err != nil {
		logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("qrLogin token fetch failed err=%v", err), "api")
		return err
	}
	var tokenResult map[string]any
	...
	logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("qrLogin token obtained uid=%s", uid), "api")
	...
	for i := 0; i < maxPolls; i++ {
		...
		if err != nil {
			logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("qrLogin poll error attempt=%d err=%v", i, err), "api")
		...
		logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("qrLogin poll attempt=%d status=%d", i, status), "api")
		switch status {
		case 0:
...
func (a *API) QRWaitAndLogin(sess *QRSession) error {
	logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("qrWaitAndLogin starting poll uid=%s app=%s", sess.UID, sess.App), "api")
	client := &http.Client{Timeout: 35 * time.Second}
	...
	for i := 0; i < maxPolls; i++ {
		...
		statusResp, err := client.Get(u.String())
		if err != nil {
			logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("qrWaitAndLogin poll error attempt=%d err=%v", i, err), "api")
			time.Sleep(time.Second)
			continue
		}
		body, _ := io.ReadAll(statusResp.Body)
		...
		logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("qrWaitAndLogin poll attempt=%d status=%d", i, status), "api")
		switch status {
		case 0:
			time.Sleep(2 * time.Second)
		case 1:
...
func (a *API) GetDirID(path string) (string, error) {
	...
	state, _ := result["state"].(bool)
	if !state {
		logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("getDirID not found (state=false) path=%s", path), "api")
		return "", nil
	}
...
func (a *API) DownloadURL(pickCode, userAgent string) (string, error) {
	...
	for attempt := 0; attempt < maxRetries; attempt++ {
		...
		if attempt < maxRetries-1 {
			logging.Log(a.logger, slog.LevelWarn, fmt.Sprintf("downloadURL network error attempt=%d/%d", attempt+1, maxRetries), "api")
			time.Sleep(time.Duration(3*(attempt+1)) * time.Second)
		}
	}
	if lastErr != nil {
...
func (a *API) ExportTree(dirID string) (string, error) {
	logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("exportTree starting cid=%s", dirID), "api")
...
func (a *API) BatchRename(renames map[string]string) (map[string]any, error) {
	...
	logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("batchRename count=%d", len(renames)), "api")
	return result, nil
}
...
func (a *API) UploadFile(localPath, targetDirID, filename string) (map[string]any, error) {
	...
	for attempt := 0; attempt < 3; attempt++ {
		...
		if err != nil {
			if attempt < 2 {
				logging.Log(a.logger, slog.LevelWarn, fmt.Sprintf("uploadFile init failed attempt=%d", attempt+1), "api")
				time.Sleep(time.Duration(3*(attempt+1)) * time.Second)
				continue
			}
			return nil, err
	...
	for attempt := 0; attempt < 3; attempt++ {
		...
		if err != nil {
			pr.CloseWithError(err)
			if attempt < 2 {
				logging.Log(a.logger, slog.LevelWarn, fmt.Sprintf("uploadFile oss failed attempt=%d", attempt+1), "api")
				time.Sleep(time.Duration(3*(attempt+1)) * time.Second)
				continue
			}
			return nil, err
		...
		if resp.StatusCode == http.StatusOK {
			...
			resp.Body.Close()
			logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("uploadFile filename=%s path=%s dir=%s", filename, localPath, targetDirID), "api")
			if data, ok := ossResult["data"].(map[string]any); ok {
				return data, nil
			}
			return ossResult, nil
		}
		logging.Log(a.logger, slog.LevelWarn, fmt.Sprintf("uploadFile failed filename=%s status=%d", filename, resp.StatusCode), "api")
		resp.Body.Close()
...
func (a *API) RapidUpload(dirID, filename string, fileSize int64, fileSHA1 string, fileStream io.ReadSeeker) (map[string]any, error) {
	...
	for attempt := 0; attempt < 3; attempt++ {
		logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("rapidUpload filename=%s attempt=%d", filename, attempt+1), "api")
		...
		switch int(status) {
		case 2:
			logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("rapidUpload success filename=%s pickcode=%v", filename, result["pickcode"]), "api")
		...
		case 7:
			...
			fallthrough
		default:
			logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("rapidUpload needs actual upload filename=%s status=%d", filename, int(status)), "api")
			return map[string]any{"status": int(status)}, nil
		}
	}
	logging.Log(a.logger, slog.LevelDebug, fmt.Sprintf("rapidUpload exceeded retries filename=%s", filename), "api")

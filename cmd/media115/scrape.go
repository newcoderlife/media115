...
Use --limit N to scrape only the first N files.`,
	...
	cmd.Flags().Bool("force", false, "重新刮削已有NFO的文件")
	cmd.Flags().Int("limit", 0, "最多刮削N个文件（0表示全部）")
	return cmd
}

func scrapeRun(o *scrapeOpts) error {
	w := o.Out
	category := o.Category
	force := o.Force
	limit := o.Limit
	...
	if len(videos) == 0 {
		fmt.Fprintf(w, "'%s' 分类中没有需要刮削的文件。\n", category)
		return nil
	}

	// Determine output dir.
	outDir := filepath.Join(o.CacheDir(), "scrape_output", category)
	...
	logFile := filepath.Join(logDir, fmt.Sprintf("scrape_%s_%d.json", category, o.Now().Unix()))
	...
	if okCount > 0 {
		fmt.Fprintf(w, "下一步: 运行 'media115 organize %s' 预览重命名计划。\n", category)
	}
	return nil
}
